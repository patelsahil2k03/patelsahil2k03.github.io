...
- **L1 — Brainstorming/design pass — ✅ COMPLETE (2026-08-30).** Full design spec written and approved: `docs/superpowers/specs/2026-08-30-3d-avatar-companion-design.md`. Key decisions locked in there: a 2-stage asset pipeline (Nano Banana stylized reference → Meshy multi-view image-to-3D, style-preserving), on-by-default with an opt-out toggle, passive/ambient interaction for v1 (no click/hover yet), **a fixed-corner companion (not a full scroll-synced traveling character)**, live simplified canvas on mobile (not a static fallback), assistant-drafted/user-approved narration copy. The original "roams the portfolio" traveling-companion vision is deliberately deferred, not dropped — documented as a future enhancement in the spec's §7.
- **L2 — Prototype spike.** Generate one character via the human-driven Nano Banana → Meshy pipeline (spec §6), get one idle + one pose-transition animation working in an isolated test page (not merged into the live site yet). Validate visual quality, file size, and frame-rate budget before scoping further.
- **L3 — Section-reactivity system** *(revised scope — no longer a full scroll-to-3D-position sync, since L1 chose the fixed-corner approach over the traveling-companion approach)*. Build the `useActiveSection()` shared hook (extracted from `Navigation.tsx`'s existing inline logic, per the spec §4) and wire it to drive `AvatarScene`'s pose/animation crossfades.
- **L4 — Narration content system.** Data-driven `src/data/avatarNarration.ts` (spec §4) mapping each section to the character's pose + thought text — assistant-drafted per Task 30's voice work, user-approved before shipping.
- **L5 — Performance/accessibility hardening.** Reduced-motion fallback, WebGL feature detection, lazy-loading, real mobile device testing (live simplified canvas, not a static fallback — see spec §3), opt-out toggle — per the spec's constraints (§2).
...
- **N1 — Audit current case-study content for contribution clarity.** Read all 10 case studies in `src/data/caseStudies.ts` fully (7 original + 3 added in Task 29), flag any section that reads as team/company-attributed rather than clearly "I built/owned this," propose specific rewrites per study. This is a content task, can happen independently of any visual redesign.
...
### Task O4: Implement approved dark mode + typography direction — ✅ COMPLETE (2026-08-30)

**Files:** `tailwind.config.ts`, `src/app/globals.css`, `src/app/layout.tsx`, `src/components/ui/ThemeToggle.tsx` (new), `src/components/ui/Navigation.tsx`, plus 20 components with color-token application (all `sections/*.tsx`, `Badge.tsx`/`Button.tsx`/`Card.tsx`/`Footer.tsx`/`Timeline.tsx`, all `case-studies/*.tsx`).

- [x] **Step 1 (revised): semantic CSS-variable tokens, not per-element `dark:` classes.** Given 237 color-class occurrences across 21 files, hand-retrofitting a `dark:` sibling onto each wasn't the right call — instead defined `:root`/`.dark` CSS custom properties in `globals.css`, registered as Tailwind colors in `tailwind.config.ts` backed by those variables (`rgb(var(--x) / <alpha-value>)`), so `.dark` on `<html>` flips every token automatically. Space Grotesk registered as `fontFamily.display`; JetBrains Mono's declaration fixed to actually load (was declared in config but never imported in `layout.tsx` — a real, previously-undiscovered bug).
- [x] **Step 2:** `ThemeToggle.tsx` built (sun/moon, `localStorage` + `prefers-color-scheme` fallback), mounted in `Navigation.tsx` always-visible before the mobile hamburger. Blocking inline script added to `layout.tsx`'s `<head>` — no flash-of-wrong-theme, verified via screenshot.
- [x] **Step 3:** All 21 flagged files swept — `blue-*`/`orange-*`/`cyan-*` → `signal-blue`/`alert-amber`(-icon)/`status-teal`(-icon) per the two-tier light-mode rule. Two real contrast bugs the substitution introduced were caught and fixed (Button secondary variant, Badge default variant tint) — see commit `ec567f6` for detail.
- [x] **Step 4-5:** Space Grotesk applied to hero name + section `<h2>`s only (narrower than "every heading" — excludes eyebrow labels and page `<h1>`s, a deliberate scope call). JetBrains Mono + `tabular-nums` on `StatCard` numerals. Hero signature gradient retuned `signal-blue → status-teal → signal-blue`.
- [x] **Step 6:** `npx tsc --noEmit`, `npm run lint`, `npm run build` all clean — independently re-run by the controller, not just trusted from the implementer's self-report. Contrast re-verified against final shipped values (found signal-blue's dark value needed a small nudge, `#3B82F6`→`#4088F7`, to clear AA on card surfaces — documented inline in `globals.css`). Screenshots taken (light/dark homepage, dark case-study page, toggle focus ring) and reviewed.
- [x] **Step 7: Approved and committed** — 4 atomic commits, `9177eab` (tokens), `c26bcd9` (toggle + persistence), `ec567f6` (component-wide application), `179225b` (an unrelated incidental `caniuse-lite` lockfile bump, split out to keep history clean). Pushed to `dev`.

**Process note:** the implementer subagent hit a session rate limit mid-task (right after finishing `ThemeToggle.tsx`, before mounting it) — resumed via `SendMessage` to its `agentId` with full context intact rather than re-dispatching fresh, so no work was lost or redone.
